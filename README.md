...
在 `Locales\NamesTranslation\预置翻译` 下增加新目录即可。公司名的行数不能改变，其他翻译可多可少。

`官译` 列没有实际作用，仅供参考。加行的时候，官译可以留空。

如果要写入繁体中文，使用 excel 编辑时，请务必保证文件是 UTF-8 编码。简体中文配置了 GB2312 解码器，但繁体中文未配置。
